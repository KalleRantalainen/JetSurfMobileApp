...
type LogFile = { name: string; bytes: number[] };
...
export default function HomeScreen() {
  ...
  function handleLogPacket(bytes: Uint8Array) {
    try {
      ...
      if (packet.type === 0x10) { logFiles.set(packet.fileIndex, { name: bytesToUtf8(packet.payload), bytes: [] }); setDownloadStatus(`Receiving ${bytesToUtf8(packet.payload)}`); }
      else if (packet.type === 0x11) { const file = logFiles.get(packet.fileIndex); if (!file || packet.offset !== file.bytes.length) throw new Error('Log packet offset gap detected'); file.bytes.push(...packet.payload); }
      else if (packet.type === 0x12) { const file = logFiles.get(packet.fileIndex); if (!file || packet.offset !== file.bytes.length) throw new Error('Log file length mismatch'); }
